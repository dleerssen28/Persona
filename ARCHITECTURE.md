...
The trait columns form the core "Taste DNA" vector. Each value is a float between 0.0 and 1.0, where 0.5 represents a neutral baseline. The `userId` column has a unique constraint, enabling upsert semantics where repeated profile updates replace the previous values.
...
Items represent recommendable content. Each item carries its own trait vector, representing the "personality" of that piece of content. A sci-fi movie might have high `traitNovelty` (0.85) and `traitAdventure` (0.9), while a cozy simulation game would have high `traitCozy` (0.95) and low `traitIntensity` (0.1).
...
Events carry trait vectors just like items, enabling the same taste-based scoring algorithm to be applied uniformly. An NBA game will have high `traitIntensity` (0.9) and `traitSocial` (0.85), while a cozy movie night will have high `traitCozy` (0.9) and low `traitIntensity` (0.3).
...
## 5. The Taste Engine: Algorithms and Scoring

The taste engine (`server/taste-engine.ts`) is the computational core of Persona. It contains no machine learning models, neural networks, or external AI services. Instead, it implements a deterministic, geometric scoring system built on distance metrics in an 8-dimensional trait space.
...
- **No machine learning models are trained or deployed.** The system uses deterministic geometric scoring (RMS distance) with hand-tuned trait vectors. There are no neural networks, gradient descents, embedding spaces, or model weights.

- **No collaborative filtering.** Recommendations are purely content-based. The system does not use "users who liked X also liked Y" patterns. Each user's recommendations depend only on their own profile, not other users' behavior.

- **No external AI/ML services.** No OpenAI, no TensorFlow, no recommendation-as-a-service APIs. The intelligence is entirely in the trait algebra.
...
- **No message queues or background workers.** All computation happens synchronously within request handlers.

- **No geolocation.** Events have text-based location strings but no coordinate-based proximity filtering.

The system demonstrates that meaningful personalization and explainable matching can be achieved with straightforward linear algebra (vector distance in a low-dimensional space), without the complexity and opacity of ML-based approaches. The trade-off is that trait vectors must be manually assigned to content items, which limits scalability but maximizes explainability.
